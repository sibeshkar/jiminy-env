...
import jiminy
from jiminy.rewarder import remote
from jiminy import twisty, wrappers
...
def safe_execute(browser, cmd, default=None):
    ...
    try:
        ...
        print("[DEBUGGING] Here's the bug shorty! ", e)
        logger.info(e)
        return "crashed"
...
class EnvController(threading.Thread):
        ...
        try:
            while True:
                ...
                if self.env_status.env_state == 'resetting':
                    self.reset()
                if self.env_status.env_state == 'browser resetting':
                    self.browser_reset()
    ...
    def launch_server(self):
        if self.setting['scheme'] == 'miniwob':
            ...
            self.server = ProxyController(mode=self.mode,
                                          ...
                                          rewarders=rewarders)
            self.server.start()
            logger.ioptions = webdriver.firefox.options.Options()
        if self.setting['scheme'] == 'realwob':
            # disable browser cache.
            options.add_argument('--disable-application-cache')
            #firefox_options.add_argument('--proxy-server=0.0.0.0:8888')
            options.add_argument('--ignore-certificate-errors')
            # if self.setting.get('device'):
            #     print('setting device', self.setting['device'])
            #     firefox_options.add_experimental_option('mobileEmulation',
            #                                            WEBDRIVER_DEVICES[self.setting['device']])nfo('cache proxy server started - mode = %s', self.mode)
    ...
    def launch_browser(self):
        ...
        print('Launching new Firefox process...')
    ...
    def trigger_browser_reset(self):
        logger.info('Triggering a reset on Browser of EnvController')
        with self.cv:
            self.env_status.set_env_info('browser resetting')
            self.cv.notify()

    def browser_reset(self):
        env_info = self.env_status.env_info()
        assert env_info['env_state'] == 'browser resetting', 'Env state should be browser resetting but is instead: {}'.format(env_info['env_state'])
        ...
        while True:
            try:
                print("[RESETTING BROWSER] Launching browser now : RESET")
                self.launch_browser()
                break
            except Exception as e:
                logger.warn('Browser reload failed after crash: %s', str(e))
                time.sleep(1.)
                #self.error_buffer.record(e)
        
        while self.browser is None:
            logger.info('Waiting for browser to reload after crash..')
            time.sleep(0.25)
...
class Rewarder(threading.Thread):
    ...
    def _read_client_reward(self, browser):
        ...
        #print("[DEBUGGING] Before Safe Execute")
        reward = safe_execute(browser, """ return WOB_REWARD_GLOBAL; """, default=0)
        safe_execute(self.env_controller.browser, """ window.WOB_REWARD_GLOBAL = 0;""")
        #print("[DEBUGGING] After Safe Execute")
    ...
    def reset(self):
        ...
        while self.env_controller.env_status.env_state == 'browser resetting':
            logger.info('Rewarder waiting for browser to be reset before resetting')
            time.sleep(0.1)


        while self._read_client_done(self.env_controller.browser):
            logger.warn('Waiting for client to reset')
            #print(self.env_controller.env_status.env_state)
            if self._read_client_done(self.env_controller.browser) == "crashed":
                logger.warn('[DEBUGGING] Browser crashed while env resetting')
                print('[DEBUGGING] : The Browser is :', self.env_controller.browser)
                self.crashes +=1
            if self.crashes >= 20: #TODO:This is a hack, only after 20 crashes does it reset browser. Otherwise premature resetting occurs 
                print("Crashes equal to or greater than 20, resetting browser")
                self.reset_browser()
                self.crashes = 0
            #    time.sleep(1)
            time.sleep(0.25)
    ...
    def reset_env(self):
        ...
        while self.env_controller.env_status.env_state == 'browser resetting':
            logger.info('[RESET ENV] Rewarder waiting for browser to be reset')
            time.sleep(0.1)

        while self.env_controller.env_status.env_state == 'resetting':
            logger.info('[RESET ENV] Rewarder waiting for env to be reset')
            time.sleep(0.1)

        self.reset()

    def reset_browser(self):
        self.env_controller.trigger_browser_reset()

        while self.env_controller.env_status.env_state == 'browser resetting':
            logger.info('[RESET BROW] Rewarder waiting for browser to be reset')
            time.sleep(0.1)

        while self.env_controller.env_status.env_state == 'resetting':
            logger.info('[RESET BROW] Rewarder waiting for env to be reset')
            time.sleep(0.1)
    ...
    def do_run(self):
        ...
        while True:
            ...
            if reward_client == "crashed" or done_client == "crashed":
               print("[DEBUGGING] Browser has crashed")
               reward_client , done_client = None , 0
               self.reset_browser()
               n = 0
               t0 = time.time()
...
def main():
    ...
    error_buffer = jiminy.utils.ErrorBuffer()
    ...
    env_status = jiminy.rewarder.EnvStatus()
